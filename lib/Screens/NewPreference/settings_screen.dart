...
class SettingsScreen extends StatefulWidget {
  final int userId;
  final dynamic value;
  const SettingsScreen({super.key, this.value, required this.userId});
...
class _SettingsScreenState extends State<SettingsScreen> with TickerProviderStateMixin{
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: TabBar(
                          ...
                          isScrollable: true,
                          onTap: (value) async{
                            if(selectedSetting == 2 && [1,2].contains(Provider.of<OverAllUse>(context, listen: false).controllerType)) {
                              await Future.delayed(Duration.zero, () {
                                setState(() {
                                  mqttPayloadProvider.viewSettingsList.clear();
                                });
                            ...
                            for(var settingIndex = 0; settingIndex < settingList[categoryIndex].setting.length; settingIndex++)
                              if(settingList[categoryIndex].setting[settingIndex].title == "RTC TIMER")
                                ...
                                Card(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Column(
                                        children: [
                                          for (int index = 0; index < preferenceProvider.individualPumpSetting!.where((e) => e.deviceId == preferenceProvider.commonPumpSettings![pumpIndex].deviceId).length; index++)
                                            Container(
                                              ...
                                              child: Column(
                                                children: [
                                                  Row(
                                                    ...
                                                    children: [
                                                      Text(preferenceProvider.individualPumpSetting!.where((e) => e.deviceId == preferenceProvider.commonPumpSettings![pumpIndex].deviceId).elementAt(index).name),
                                                      Switch(
                                ...
                                buildCustomListTileWidget(
                                    ...
                                    widgetType: (settingList[categoryIndex].setting[settingIndex].title == "SENSOR HEIGHT" || settingList[categoryIndex].setting[settingIndex].title == "PRESSURE MAXIMUM VALUE")
                                        ? 7
                                    ...
                                    ? [0, 1, 2].contains(settingIndex) ? [0, 1, 2][settingIndex] : 0
                                        : categoryIndex == 1
                                    ? [0, 1, 2].contains(settingIndex) ? [0, 1, 2][settingIndex] : 0
                                    : [0, 1, 2,3,4,5].contains(settingIndex) ? [0, 1, 2,3,4,5][settingIndex] : 0]) : "Loading..."}" : null,
          ...
          for (var element in decodedList) {
            ...
            decode.forEach((key, value) {
              switch (type) {
                ...
                case 26:case 206:
                  print("key ==> $key");
                  if (key.contains("ctconfig")) {
                    valueToShow = value;
                    print("$key ==> $valueToShow");
                  };
                  break;
                case 24:case 204:
  ...
  bool getConditionToShow({required int type, required int serialNumber, required value}) {
    ...
    void setCondition(String key) {
      conditions[key] = value is bool ? value : true;
    }
    switch (type) {
      case 26:case 206:
