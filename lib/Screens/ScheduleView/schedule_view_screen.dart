...
class _ScheduleViewScreenState extends State<ScheduleViewScreen> {
  ...
  Future<void> _requestScheduleData() async {
    MqttManager().schedulePayload = null;
    ...
    var getUserConfigMaker = await repository.getUserConfigMaker(userData);
    ...
    final headUnitSnoList = MqttManager().schedulePayload!.map((e) => e['HeadUnit'].toString()).toList().toSet();
    final programSnoList = MqttManager().schedulePayload!.map((e) => e['ProgramS_No'].toString()).toList().toSet();
    final statusCodeList = MqttManager().schedulePayload!.map((e) => e['Status']).toList().toSet();
    ...
    print('programs: ${programs.toSet().toList()}');
    print('statusList: ${statusList.map((e) => e.statusString)}');
    print('programCategories: ${headUnits.toSet().toList()}');
    Map<String, dynamic> data = {
      "2600": {"2601": "${DateFormat('yyyy-MM-dd').format(date)},${DateFormat('yyyy-MM-dd').format(date)}"}
    };
    MqttManager().topicToPublishAndItsMessage('${Environment.mqttPublishTopic}/${widget.deviceId}', jsonEncode(data));
  }
  ...
  Future<void> _getUserSequencePriority(userId, controllerId) async {
    try {
      ...
      if (getUserScheduleLog.statusCode == 200) {
        ...
        MqttManager().schedulePayload = Constants.dataConversionForScheduleView(convertedJson['data']['log'][0]['irrigation']);
      }
    } catch (e, stackTrace) {
      print('Error: $e');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
              ...
              Expanded(
                child: Center(
                  child: StreamBuilder<List<Map<String, dynamic>>?>(
                    stream: MqttManager().schedulePayloadStream,
      ...
      floatingActionButton: FilledButton.icon(
        onPressed: () async {
          // var
          final convertedList = MqttManager().schedulePayload!;
          var listToMqtt = [];
      ...
      children: [
        ...
        Container(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              return _buildOutlineButton(headUnits[index], selectedHeadUnits, item: headUnits[index]);
            },
          ),
        ),
        ...
        Container(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              return _buildOutlineButton(programs[index], selectedPrograms);
            },
          ),
        ),
        const SizedBox(height: 16),
