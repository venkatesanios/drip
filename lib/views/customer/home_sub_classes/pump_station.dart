...
class PumpStation extends StatelessWidget {
  ...
  Widget buildMobile(BuildContext context, List<WaterSource> sortedWaterSources, PumpStationViewModel vm) {
    ...
    for (int i = 0; i < sortedWaterSources.length; i++) {
      var source = sortedWaterSources[i];

      gridItems.add(SourceWidget(source: source, index: i));

      for (int j = 0; j < source.outletPump.length; j++) {
        var pump = source.outletPump[j];
        gridItems.add(PumpWidget(pump: pump, vm: vm));
      }
    }

    for (var filterSite in vm.mvFilterSite) {
      for (var filter in filterSite.filters){
        gridItems.add(FilterWidget(filter: filter));
      }
    }
